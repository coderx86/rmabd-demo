...
export function EventSegments() {
  ...
  return (
    <section id="event-segments" className="py-24 bg-background w-full overflow-x-hidden">
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-10">
          {segments.map((segment) => (
            <Card
              key={segment.id}
              className="group hover-transition hover:shadow-lg bg-card border-border overflow-hidden w-full"
            >
              <CardContent className="p-0">
                ...
                <div className="p-6">
                  <h3 className="text-xl font-semibold text-foreground mb-3">{segment.name}</h3>
                  <p className="text-muted-foreground mb-6 text-sm">{segment.description}</p>
          ...
          {selectedSegment && (
            <>
              <DialogHeader className="p-4 sm:p-6 border-b border-border">
                <DialogTitle className="text-xl sm:text-2xl font-bold pr-8">{selectedSegment.name}</DialogTitle>
              </DialogHeader>
              ...
              <div className="p-4 sm:p-6 space-y-6">
                ...
                <div className="bg-gradient-to-br from-primary/10 to-accent/10 h-48 sm:h-64 rounded-lg flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <h3 className="text-lg sm:text-2xl font-bold text-foreground px-4">{selectedSegment.name}</h3>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row sm:flex-wrap gap-4 sm:gap-6 text-sm text-muted-foreground">
                  <div className="flex items-center space-x-2">
                    ...
                    <span>{selectedSegment.details.date}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Clock className="h-4 w-4 text-primary flex-shrink-0" />
                    <span>{selectedSegment.details.duration}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <MapPin className="h-4 w-4 text-primary flex-shrink-0" />
                    <span>{selectedSegment.details.location}</span>
                  </div>
                </div>

                {/* Instructions */}
                <div>
                  <h3 className="text-base sm:text-lg font-semibold mb-3">Instructions</h3>
                  <div className="bg-muted p-3 sm:p-4 rounded-lg">
                    <p className="text-light-black dark:text-light-white leading-relaxed text-sm sm:text-base">
                ...
                <div className="bg-primary/10 p-3 sm:p-4 rounded-lg">
                  <h3 className="text-base sm:text-lg font-semibold text-primary mb-2">Registration Deadline</h3>
                  <p className="text-foreground text-sm sm:text-base">{selectedSegment.details.deadline}</p>
                </div>
